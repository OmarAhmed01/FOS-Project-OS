...
void page_fault_handler(struct Env * curenv, uint32 fault_va) {
	...
	int place_max = curenv->page_WS_max_size;
	uint32 addrVirt =
			curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address;
	uint32 VAddr;
	uint32 *table_ptr = NULL;
	struct Frame_Info *farme_info = NULL;
	int operation = 1;

	if (size < place_max) {
		page_allocation(curenv, fault_va);
	} else {
		//tmodclk take 12
		...
		if (isPageReplacmentAlgorithmModifiedCLOCK()) {
			int true = 1;
			while (true) {
				uint32 start_i = curenv->page_last_WS_index;
				uint32 VAddr_index = start_i;
				for (int max = curenv->page_WS_max_size; max > 0;
						max--, VAddr_index++) {
					VAddr_index = (VAddr_index % curenv->page_WS_max_size);
					uint32 temp_VA = env_page_ws_get_virtual_address(curenv,
							VAddr_index);
					...
					if ((page_permissions & PERM_MODIFIED) == 0
						...
						VAddr = temp_VA;
						victim_index = VAddr_index;
						curenv->page_last_WS_index = ((VAddr_index + 1)
		...
		//khaly balak mn soso
		farme_info = get_frame_info(curenv->env_page_directory, (void *) VAddr,
				&table_ptr);
		if ((pt_get_page_permissions(curenv, VAddr) & PERM_MODIFIED)
				== PERM_MODIFIED) {
			pf_update_env_page(curenv, (void*) VAddr, farme_info);
		}
		unmap_frame(curenv->env_page_directory, (void*) VAddr);
		env_page_ws_invalidate(curenv, VAddr);
		page_allocation(curenv, fault_va);
	}
}
...
void page_allocation(struct Env * curenv, uint32 fault_va) {
	struct Frame_Info *frame_info = NULL;
	int frame_check = allocate_frame(&frame_info);
	uint32 page_permissions = pt_get_page_permissions(curenv, fault_va);
	page_permissions |= (PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
	if (frame_check != E_NO_MEM) {
		map_frame(curenv->env_page_directory, frame_info, (void*) fault_va,
				page_permissions);
		frame_check = pf_read_env_page(curenv, (void *) fault_va);
		if (frame_check == E_PAGE_NOT_EXIST_IN_PF) {

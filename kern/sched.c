...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel) {
	...
	int j = 0;
	while (j < num_of_ready_queues) {
		*quantums = *quantumOfEachLevel;
		quantums++;
		quantumOfEachLevel++;
		init_queue(&env_ready_queues[j]);
		env_ready_queues++;
		j++;
	}
	int i = 0;
...
struct Env* fos_scheduler_MLFQ() {
	...
	if (curenv != NULL) {
		if (envi >= num_of_ready_queues - 1) {
			enqueue(&env_ready_queues[envi], curenv);
		} else {
			enqueue(&env_ready_queues[envi + 1], curenv);
		}

	}
	struct Env* next_env = NULL;
	int k = 0;
	// search in all queues until he finds environment ready to proccess
	//if found ...
	//1)let next env pointer point at the ready env to work
	//2)set cpu clock by quantum of the level he's in
	while (k < num_of_ready_queues) {

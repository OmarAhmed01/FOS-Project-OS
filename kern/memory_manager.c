...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32 * ptr_table_start = kmalloc(PAGE_SIZE);

	struct Frame_Info* ptr_frame_info = get_frame_info(ptr_page_directory, (void *)ptr_table_start, &ptr_page_table);

	uint32 phys_page_table = to_physical_address(ptr_frame_info);
	//*ptr_page_table = kheap_physical_address(phys_page_table) ;

	//initialize new page table by 0's
	memset(ptr_table_start , 0, PAGE_SIZE);
...
{
	uint32* ptr_env_page_table = NULL;
	uint32 c = 0;
	uint32 v = virtual_address;
	int a = 0, b = 0, m = 0;
	for (int i = 0; i < size; i += PAGE_SIZE) {
		for (int var = 0; var < e->page_WS_max_size; var++) {
			if (e->ptr_pageWorkingSet[var].virtual_address == v) {
				env_page_ws_clear_entry(e, var);
	...
	v = virtual_address;
	for (int i = 0; i < size; i += PAGE_SIZE) {
		get_page_table(e->env_page_directory, (void*) v, &ptr_env_page_table);
		if (ptr_env_page_table != 0) {
			c = 0;
			uint32 p;
			for (p = 0; p < 1024; p++) {
				if ((ptr_env_page_table[p] & PERM_PRESENT) == 0)
					c++;
				else
					break;
			}
			if (c == 1024) {
				kfree((void*) ptr_env_page_table);
				e->env_page_directory[PDX(v)] = 0;
			}
		}
		v += PAGE_SIZE;
	}
